...
from AliciaMusic.misc import HAPP, SUDOERS, XCB
from AliciaMusic.utils.database import (get_active_chats,
                                       remove_active_chat,
                                       remove_active_video_chat)
from AliciaMusic.utils.decorators.language import language
...
async def vardel_(client, message, _):
    ...
    if await is_heroku():
        ...
        if not output[0]:
            return await message.reply_text(_["heroku_4"])
        await message.reply_text(_["heroku_7"].format(check_var))
        os.system(f"kill -9 {os.getpid()} && python3 -m AliciaMusic")
